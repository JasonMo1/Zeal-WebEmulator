...
(function (mod) {
        ...
        init: function (pane, orig, options) {
            ...
            if (this.mv.options.connect == "align") {
                if (!this.edit.state.trackAlignable)
                    this.edit.state.trackAlignable = new TrackAlignable(
                        this.edit,
                    );
    ...
    function alignChunks(dv, force) {
        ...
        for (var i = 0; i < cm.length; i++) {
            ...
            offset.push(
                -cm[i].getScrollerElement().getBoundingClientRect().top,
            );
        }

        if (
            offset[0] != offset[1] ||
            (cm.length == 3 && offset[1] != offset[2])
        )
        ...
        if (dv.copyButtons) {
            ...
            copy.style.top =
                (chunk.origTo > chunk.origFrom
                    ? top
                    : dv.edit.heightAtLine(chunk.editFrom, "local") -
                      sTopEdit) + "px";
    ...
    function copyChunk(dv, to, from, chunk) {
        ...
        if (handler)
            ...
            to.replaceRange(
                from.getRange(origStart, origEnd),
                editStart,
                editEnd,
            );
    }
        ...
        hasWidget: function (n) {
            ...
            if (handle.widgets)
                for (var i = 0; i < handle.widgets.length; i++)
                    if (
                        !handle.widgets[i].above &&
                        !handle.widgets[i].mergeSpacer
                    )
                        return true;
            return false;
        },
